...
- **フロントマターの `output:` を省略できるようになりました。** 書かなければ、
  **原稿と同じ場所に同じ名前の `.pptx`** ができます（`deck.md` → `deck.pptx`）。
  カレントディレクトリではなく原稿の隣に置くので、別の場所から
  `md2pptx sub/deck.md` と打っても pptx は原稿と一緒にできます。
  必須のキーは `theme` だけになりました。`-o` とフロントマターの優先順位は
  変わっていないので、書いてあるデッキはそのまま動きます [#189]

### 変更

- **出力名が `.pptx` で終わらないときは付け足すようになりました**（`deck` →
  `deck.pptx`）。付け足したことは警告に出ます。置き換えではなく付け足しなのは、
  `v1.2-deck` のような名前で `.2-deck` を拡張子と見なして消してしまわないためです [#189]
- **入力を上書きする指定を拒むようになりました。** `.pptx` という名前で Markdown を
  書いていた場合、これまでは原稿が pptx で置き換わっていました（実測で 143 バイトの
  原稿が 31KB の pptx に）。同じファイルを指していれば
  `refusing to overwrite the input` で止まります [#189]
...
- `<!-- @table-width: 70% -->` で**表の総幅**を帯より狭くできるようになりました。
  狭めたぶんは左右へ等分され、表は帯の中で中央に置かれます。`auto` と書くと、
  列ごとに最長のセルが1行に収まる幅を積み上げます。短いセルばかりの対応表が
  横に間延びするのを抑えられます [#173]

### 修正

- 縦並びの ` ```flow ` で、**box の幅が中身に関係なく同じ**でした。長い名前が
  折り返して box が縦に伸び、図が窮屈になっていました。いちばん長いラベルが
  1 行に収まる幅にし、矢印ラベルが入るすき間もその高さぶん空けます [#185]

- ` ```seq ` の中で `@step` を使うと、**段が進むたびに図が上下に動いて**いました。
  矢印の少ない段ほど間隔が広がっていたためです。最終段の本数で送り幅を決めるように
  したので、下に矢印が増えていくだけに見えます [#183]

- ` ```flow ` の**矢印ラベルが box に重なって**いました。ラベルの枠を 16pt 前提で
  見積もりながら、実際は本文の標準サイズ（30pt になることもあります）で描いて
  いたためです。枠は上下中央ぞろえなので、大きい字は枠の外へはみ出して下の box に
  乗っていました。描くサイズで枠を決めるようにしました [#179]
...
- `{box}` の枠が、囲んでいる字に対して**少し下へ寄って**見えていました。
  ...
  枠を持ち上げました [#161] [#165]
- 図のキャプションが折り返すと、**2行目が下の罫線やページ番号に掛かって**
  いました。1行ぶんの高さしか確保していなかったためです。折り返す行数で
  確保するようにしました（長く書けばそのぶん図が小さくなります） [#170]
- ` ```seq ` の注記やラベルが**スライドの外へはみ出して切れる**ことがありました。
  場所は 16pt 想定で計算しているのに、描くのは本文標準サイズだったためです。
  本文の大きいテーマで出ます [#168]
- 行数の少ない表が**帯いっぱいに引き伸ばされ**、1行が 2.5cm ほどになって
  いました。見積もりが帯より小さければそちらの高さで描き、余った空きは上下へ
  均等に配るようにしました [#166]
...
[#189]: https://github.com/toshi0806/md2pptx/pull/189
