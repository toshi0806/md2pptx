...
`saved:` の 2 行だけ `md2pptx:` が付いていませんが、書き間違いではありません。
**成果物のパスは標準出力、進捗は標準エラー**という分け方で、
`md2pptx … | grep '^saved:'` のように機械で拾えます。端末では両方が混ざって見えます。
...
**確認できたらパスを元に戻して保存してください**（「問題」から消えます）。戻さないと、
以降のビルドは失敗し続けて PDF が更新されません。
