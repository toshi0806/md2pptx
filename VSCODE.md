...
ファイル名は2通り出てきます。**`slide.md` は自分の原稿**——**実際のファイル名に読み替えてください**。
...
`saved:` の2行だけ `md2pptx:` が付いていないのは意図的です。
...
**「問題」パネル**（`Cmd+Shift+M` で開く）に出ます。このリポジトリの `example.md` で
...
別の Markdown に切り替えるときも、いったん止めてから手順1に戻ってください（`${file}`
はタスク実行時に固定されるため）。原稿が1つに決まっているなら、`${file}` の代わりに
...
`runOptions.runOn` を使うと、**フォルダを開いた時点で watch が始まります**。
手順1〜3が丸ごと不要になるので、原稿が1つに決まっているならこちらが快適です。

これは**貼り付ける断片ではなく、書き換えの指示**です。上の
`md2pptx: watch` タスクのうち次の3つだけを変え、**残り（`label` / `type` / `command` /
`isBackground` / `presentation` / `problemMatcher`）はそのまま**にしてください。
...
- **タスクが動いているか**（手順3）。
- **PDF のタブをクリックしてからタスクを実行していないか**（手順1）。
  そうすると watch は Markdown ではなく PDF を見張っています。いったん止めて、
  Markdown のタブをクリックしてから実行し直してください。
- **PDF のタブが LaTeX Workshop のビューアで開かれているか。** エディタ右上が
  PDF 表示ではなくテキストや別のビューアなら、[用意するもの](#用意するもの)
  の関連付けが効いていません。
...
Terminal で承認済みでも VS Code では改めて出ます。押し損ねても180秒で諦めるだけなので、
