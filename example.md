...
→ `@table-width` は `auto` のほか `70%` のような割合でも指定できる
...
1. {box:blue} いま話しているのはこの項目
2. シラバスや講義日程で「今日はここ」を示す
3. 囲むのは段落なので、折り返しても枠は1つ
4. {box} 色を書かなければテーマのアクセント色

→ 相対サイズと併記でき、順序は問わない（`{box}{-1}` も `{-1}{box}` も同じ）
...
info string に `color` を足すと、そのフェンスの中でだけ `[語]{色}` が効く
...
→ 装飾記号は桁に数えないので**等幅の桁揃えは崩れない**

### 段階表示：@step でアニメーションの代わり

- アニメーションは書かず、1枚分の原稿から複数枚を作る
<!-- @step -->
- `<!-- @step -->` の位置で1枚切り出し、続きを積み上げる
<!-- @step -->
- タイトルとディレクティブは全段で共通、ノートは最終段だけ

→ このスライドは3枚に展開されている

```note
段階表示の例です。ここまでのスライドと違い、1つの見出しから3枚が生成されています。

ノートは最終段だけに付くので、発表者ビューにこの原稿が3回出ることはありません。
```
