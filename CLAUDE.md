...
しても画面が乱れない。**表示されてしまう 2 ケース**は仕様なので直そうとしないこと: 利用者が
既に PowerPoint を表示して使っている（`-j` は起動の瞬間にしか効かない）／TCC 未承認フォルダ
での初回実行（powerbox の許可ダイアログは応答が要る）。`save … as PDF` は隠したアプリを自ら
