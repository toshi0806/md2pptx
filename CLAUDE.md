...
**出力 pptx / PDF は必ず作業ディレクトリ経由で `os.replace` する**（`Renderer.save` /
`pdf.convert`）。pptx 側の理由は python-pptx が `zipfile.ZipFile(path, "w")` で書くこと
——開いた瞬間に出力先を切り詰めるので、直接書くと作り直している間だけ壊れた pptx が
見える（実測: 25 回連続ビルド中 38957 サンプル中 215 回。差し替え方式では 0 回）。
