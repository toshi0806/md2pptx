...
枠を消して作り直したテーマでは別の番号になりうるので、`_body_placeholder` が None を返す。
**止めはしない**——タイトルも表も出せるし pptx の保存は成功する（「PDF が作れなくても
pptx は成功」と同じ扱い）——が、**捨てた行があれば `_warn_no_body` で何を捨てたか言う**。
黙ると、出力を開くまで気づけず、気づいてもテーマが原因だとは思い当たらない。
落とす経路は2つ（`render_slide` の通常路と `_render_stacked_into` の表・図あり）で、
**両方に通す**こと。固定しているのは `tests/test_body_placeholder_warning.py`。
...
- **文書の日本語は、描画結果が変わらない位置でだけ折る**（Issue #62）。
  ...
  **文字列としては同じまま構造だけ壊れる折り方が3つある**ので、
  ...
  3つとも実際に踏んだ。
- **`**` は記号に接して閉じられない**（Issue #77）。CommonMark
  は閉じ側を「直前が記号でない」ときだけ閉じ括弧とみなすので、`**…）**で` や
  `**…」**と` は**強調にならず `**` が文字として出る**。日本語は括弧・鉤括弧・句読点が
  多いので踏みやすく、折り方の問題ではないから前項の規則を守っても防げない。
  リンクを強調するなら `**[語](#先)**` ではなく `[**語**](#先)` と**内側に置く**。
  確認は `gh api /markdown -f text='…'` が速い（commit 前に描画できる）。
  素の `**` が残っていないかは、レンダリング結果から `<code>`・`<pre>` を除いて
  `**` を探せば洗い出せる。**`-f mode=gfm` は付けないこと**——
  段落内の改行を `<br>` にしてしまい、リポジトリの `.md`
  の描画（改行は空白）と食い違う。既定のモードが README と同じ描画で、表も出る。
- **和欧間の空白は「ラテン文字の語」との間にだけ入れる**（Issue #62）。
  ...
  **例外は引用**——UI の表示名やエラー文言など、外部から写した文字列は逐語で置く。
  PowerPoint のレイアウト名「2 つのコンテンツ」は空白を含むのが実物で、詰めると別物になる。
- **SYNTAX.md の節見出しには句読点・括弧を入れない**（Issue #77）。
  冒頭の早見表は各節へアンカーで飛ぶが、**GitHub はアンカーを作るとき記号を除去する**ので、
  `画像（jpg / png）` のような見出しでは飛び先を書き当てにくい。
  記号を含まない見出しならアンカーは見出しそのままの文字列になる。
  括弧に入れたくなった補足は、その節の1行目へ回す。
  **見出しを変えるときはリンクも直すこと**——早見表の13本と、
  `THEME.md` からの `SYNTAX.md#フロー図` の1本が指している。
  README の `#コマンドラインリファレンス` も同じ。
  GitHub が実際に生成した id と突き合わせて確かめられる。**`{owner}` と `{repo}` は
  そのまま打つ**（gh がカレントリポジトリから解決する）が、`<branch>`
  は自分で置き換える——記法が違うのは、置換するのが誰かが違うから:
