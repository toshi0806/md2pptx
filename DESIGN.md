...
- **段落の高さは「そのレベルの実サイズ × 1.32 ＋ そのレベルの `spcBef`」**
  ...
  - **`spcBef`（段落前のアキ）**。マスターの `bodyStyle` から読む
    （`_body_space_before`）。`spcPct` はフォントサイズに対する％なので、その場では
    pt に直せない——**負の値を「％である」印**にして返し、サイズを掛けるのは
    呼び出し側。OOXML に負のアキは無いので衝突しない。cn2026-theme は全レベル 20%
    を持ち、30pt の段落で 0.21cm、4段落で 0.85cm ずれていた。
