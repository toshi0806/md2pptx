...
- **枠は行の箱より 8.6% 上へ出す**（`_BOX_LIFT`．Issue #160）。PowerPoint は行の
  箱の中で字を上寄りに置き、下に descent ぶんの空きを残す。行の箱にそのまま
  合わせると字に対して枠が下がって見える——実測で上 4.8pt / 下 11pt（30pt の
  行・箱 36pt）。差の半分だけ持ち上げて上下の空きを揃える。位置によらず一定なので
  積もりはしない。
