...
- **pptx テンプレートを手で用意しない**。thmx から内部でベース pptx を生成し、それを土台に描画する。
  （従来の `demo-slide-base.pptx` の役割を thmx が直接担う。）
...
- 余裕があれば、既存の demo スライド（`参照スクリプト` 出力）を thmx＋Markdown から再現できるか検証する。

## 2. 全体アーキテクチャ

```
  OfficeTheme.pptx          input.md
            │                           │
            ▼                           ▼
 ┌────────────────────┐      ┌────────────────────────────┐
...
ステージ 3 はその base pptx に対して `add_slide` していく（従来 `demo-slide-base.pptx` を開いていた箇所が、
ステージ 0 の生成物に置き換わる）。
...
IR を受け取って描画する純粋関数群として整理する。`参照スクリプト` 自体は当面そのまま残す。

## 3. thmx → base pptx 変換（ステージ 0）

`OfficeTheme.pptx` で実証済みの手順。thmx は内部的に pptx とほぼ同型で、
...
> 本文（レベル付き箇条書き）設定 → 保存まで成功。**現行 参照スクリプト とそのまま接続できる。**
...
```yaml
---
theme: OfficeTheme.pptx   # ★ デザインのおおもと（thmx）。CLI 引数が優先
...
- 列幅は均等を既定とし、ディレクティブで比率指定できるようにする（任意）。
...
python3 md2pptx.py input.md --theme OfficeTheme.pptx -o out.pptx
```
...
1. **Phase 0（thmx 変換）**：`thmx2pptx.py` を実装し、base pptx を確実に生成（実証済みロジックの製品化）。
2. **Phase 1（最小実用）**：front matter＋タイトルスライド＋`##` 分割＋箇条書き（`-`/`1.`/`→`）＋
   自動採番＋`no_bullet`＋autofit＋スライド番号。`render.py` へヘルパー移植。
3. **Phase 2（表）**：Markdown テーブル対応、導入文＋表＋結論の混在スライド。
4. **Phase 3（図 DSL）**：` ```flow ` パーサとレイアウタ、矢印ラベル・caption・note。
5. **Phase 4（再現検証）**：demo の各スライドを Markdown 化し、`参照スクリプト` 出力と
   見比べて差分を詰める（丸数字採番・autofit 率・図の配置など）。

## 9. demo 再現の対応表（検証チェックリスト）

| 現行スライド | 使用機能 | 想定 Markdown |
|---|---|---|
| タイトル | 多段タイトル＋著者複数行 | front matter |
| 背景 | bullet 2 レベル | `-` ネスト |
| 課題の整理 | 導入文＋表＋結論 | 段落＋表＋`→` |
| 本稿の貢献 | 丸数字採番（色 tx1） | `①`＋`@autonum-color: tx1` |
| 全体構成・概観 | 一部行のみ `(1)(2)` 採番 | `(1)` マーカー混在 |
| モジュール構成 | enum_items（見出し＋説明） | `1.`＋ネスト `-` |
| 自動伝播チェーン | 全行採番＋結論 no_bullet | `1.` 連番＋`→` |
| レビュー工程 | box/arrow/note 図 | ` ```flow ` |
