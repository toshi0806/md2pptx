...
**縦並びの box は中身で幅が決まる**（Issue #184）。3.2in 固定だと長い名前が
折り返し、行数のぶん box が縦に伸びて図が窮屈になる。いちばん長いラベルが 1 行に
収まる幅を取り、下限は 3.2in（短い名前ばかりでも box が痩せない）、上限は帯。
矢印ラベルのあるすき間は、そのラベルの高さぶん空ける——すき間はラベルの中央に
合わせるので、狭いと上下の box に掛かる。
...
ファイル冒頭の `---` で囲む。**テーマ（thmx）**を持ち、出力先は任意。
表紙は本文記法で書く（§5.2）。

```yaml
---
theme: OfficeTheme.pptx            # ★ デザインのおおもと（.thmx / .pptx）。CLI 引数が優先
output: out.pptx                   # CLI 引数が優先。省略可（下記）
...
- **`output` は省略できる**（Issue #188）。`-o` もフロントマターも無ければ
  **入力と同じ場所・同じ basename の `.pptx`** を使う（`deck.md` → `deck.pptx`）。
  カレントディレクトリではなく入力の隣に置くのは、これが「パス」ではなく**入力から
  導いた名前**だから——`theme:` の相対解決や画像と同じ基準で読める。入力を書かれた
  まま使う（相対なら相対）ので `saved:` の行も利用者が打った形で出る。
  実態として手元のデッキは 14/14 が .md と同名で、書く意味のある指定ではなかった。
- **出力は必ず `.pptx` で終わる。** 違う拡張子・拡張子なしで書かれたら**末尾に足し、
  足したことを警告に出す**（黙って別の名前にはしない）。置き換えないのは、`splitext` が
  `v1.2-deck` の `.2-deck` を拡張子と見なすため——置き換えると `v1.pptx` になって
  名前が消える。これで `-o slide.md` と書かれても原稿は潰れない（`slide.md.pptx` になる）。
- **それでも入力を出力先にはしない。** 拡張子を揃えても一致しうる経路が1つ残る——
  **`.pptx` という名前で Markdown を書いた**とき。中身がテキストなら parse は成功し、
  導出した名前は入力そのものになる（実測で 143 バイトの原稿が 31KB の pptx に置き換わった）。
  比べるのはパスの文字列ではなく**実体**（`os.path.samefile`）。文字列だとシンボリック
  リンク経由も、大文字小文字を区別しないファイルシステム（macOS の既定）での `SLIDE.md` も
  別物に見える。出力がまだ無ければ `OSError` になるが、**存在しない以上それは入力ではない**。
