...
- 符号は省略可（`{2}` ＝ `{+2}`）。トークンが無い行はスライド既定（無ければテーマ既定）に従う。
- 実サイズ ＝ `round(base × 1.125**delta)`、下限 8pt。`base` はその行の `level` に対応する
  本文スタイルの既定サイズ（`_body_font_levels`）。pt 値はコードに持たず、テーマ由来の比だけを持つ。
- インデント（`p.level`）は変えず、段落の run へサイズだけを焼き込む。`@autofit` の縮小とは
