...
## 9. demo 再現の対応表（検証結果）【Phase 4 完了】

`参照スクリプト` 出力（基準 24 枚）を `demo.md`（→ `demo.pptx`）で再現し，
`ppt2pdf`（実 PowerPoint レンダリング）＋ `pdftoppm` で 1 枚ずつ突き合わせた．

| 現行スライド | 使用機能 | 想定 Markdown | 再現 |
|---|---|---|---|
| タイトル | 多段タイトル＋著者複数行 | front matter | △（副題位置・著者枠幅が異なる） |
| 背景 | bullet 2 レベル | `-` ネスト | ◎ |
| 課題の整理 | 導入文＋表＋結論 | 段落＋表＋`→` | ◎ |
| 本稿の貢献 | 丸数字採番（色 tx1） | `①`＋`@autonum-color: tx1` | ◎ |
| 全体構成・概観 | 一部行のみ丸数字採番（黒） | `①` 混在＋`@autonum-color: tx1` | ◎ |
| モジュール構成 | enum_items（見出し＋説明） | `1.`＋ネスト `-` | ◎ |
| 自動伝播チェーン | 全行採番＋結論 no_bullet | `1.` 連番＋`→` | ◎ |
| 支援基盤 | 本文縮小 | `@autofit: 90` | ◎ |
| レビュー工程 | box/arrow/note 図 | ` ```flow ` | ◎（矢印は box 高に比例した太さ） |
| 考察(1)(2) | 結論行 no_bullet | `→`（記号なし行） | ◎ |
