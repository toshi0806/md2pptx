...
def test_empty_line_is_rendered_as_a_runless_paragraph(tmp_path):
    ...
    サイズは実装の式を写さず**数値で書く**．既定テンプレートの本文 lvl1 は 32pt，
    1 段 ÷1.125 の 2 段下は 32 ÷ 1.125² ＝ 25.28pt で，``_apply_size_delta`` が
    ``round`` して **25pt**（``sz="2500"``）になる．``sz`` は 1/100 pt 単位の整数
    属性だが，md2pptx は pt 単位で丸めてから ``Pt()`` へ渡すので 25.28pt は出ない．

    比率は DESIGN.md §5.8 で決めた仕様なので，変わればこのテストが落ちるのが
    正しい——実装から計算すると式を写すだけになり，比率が変わっても通ってしまう．
    """
